...
from ArgoverseDataset.argoverse.data_loading.vector_map_loader import load_lane_segments_from_xml
from ArgoverseDataset.argoverse.utils.centerline_utils import (
...
from ArgoverseDataset.argoverse.utils.cv2_plotting_utils import get_img_contours
from ArgoverseDataset.argoverse.utils.dilation_utils import dilate_by_l2
from ArgoverseDataset.argoverse.utils.geometry import point_inside_polygon
from ArgoverseDataset.argoverse.utils.json_utils import read_json_file
from ArgoverseDataset.argoverse.utils.manhattan_search import (
    compute_polygon_bboxes,
    find_all_polygon_bboxes_overlapping_query_bbox,
    find_local_polygons,
)
from ArgoverseDataset.argoverse.utils.mpl_plotting_utils import plot_lane_segment_patch, visualize_centerline
from ArgoverseDataset.argoverse.utils.pkl_utils import load_pkl_dictionary
from ArgoverseDataset.argoverse.utils.se2 import SE2
...
MAP_FILES_ROOT = ROOT / "argoverse/map_files"
...
class ArgoverseMap:
    ...
    def __init__(self) -> None:
        ...
        # update, 210303
        self.city_to_lane_polygons_ids_dict = {}

        for city_name in self.city_name_to_city_id_dict.keys():

            # update, 210303
            lane_polygons, lane_polygons_dict = self.get_vector_map_lane_polygons(city_name)
            ...
            # update, 210303
            self.city_to_lane_polygons_ids_dict[city_name] = lane_polygons_dict


    def get_vector_map_lane_polygons(self, city_name):
        """
        ...
        """
        lane_polygons = []
        lane_polygons_dict = {} # update, 210303
        ...
        for lane_id, lane_segment in lane_segments.items():
            ...
            lane_polygons_dict[lane_segment.id] = lane_polygon_xyz # update, 210303

        return lane_polygons, lane_polygons_dict
    ...
    def find_local_lane_centerlines_with_ids(
        self,
        query_x,
        query_y,
        city_name,
        query_search_range_manhattan= 80.0):
        """
        Find local lane centerline to the specified x,y location
        ...
        lane_ids = self.get_lane_ids_in_xy_bbox(query_x, query_y, city_name, query_search_range_manhattan)
        local_lane_centerlines = [self.get_lane_segment_centerline(lane_id, city_name) for lane_id in lane_ids]
        return np.array(local_lane_centerlines), lane_ids