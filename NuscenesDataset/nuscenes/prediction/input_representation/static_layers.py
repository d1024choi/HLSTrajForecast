...
from NuscenesDataset.nuscenes.eval.common.utils import quaternion_yaw
from NuscenesDataset.nuscenes.map_expansion.map_api import NuScenesMap, locations
from NuscenesDataset.nuscenes.prediction import PredictHelper
from NuscenesDataset.nuscenes.prediction.helper import angle_of_rotation, angle_diff
from NuscenesDataset.nuscenes.prediction.input_representation.combinators import Rasterizer
from NuscenesDataset.nuscenes.prediction.input_representation.interface import \
    StaticLayerRepresentation
from NuscenesDataset.nuscenes.prediction.input_representation.utils import get_crops, get_rotation_matrix, convert_to_pixel_coords
...
class StaticLayerRasterizer(StaticLayerRepresentation):
    ...
    def make_representation(self, instance_token: str, sample_token: str) -> np.ndarray:
        ...
        for mask, color in zip(masks, self.colors):
            ...
            # cv2.imshow("", 255*mask.astype('uint8'))
            # cv2.waitKey(0)
            #
            # cv2.imshow("", 255 * mask[::-1, :, np.newaxis].astype('uint8'))
            # cv2.waitKey(0)
    ...
    def make_representation_(self, ann_token: str, sample_token: str) -> np.ndarray:
        """
        Makes rasterized representation of static map layers.
        :param instance_token: Token for instance.
        :param sample_token: Token for sample.
        ...
        if (ann_token == 'EGO'):
            lidar_now_token = self.helper.data.get('sample', sample_token)['data']['LIDAR_TOP']
            sample_data = self.helper.data.get('sample_data', lidar_now_token)
            sample_annotation = self.helper.data.get('ego_pose', sample_data['ego_pose_token'])
        else:
            sample_annotation = self.helper.data.get('sample_annotation', ann_token)
